...
class ToolCallLogger:
    ...
    def log_call(self, tool_name: str, args: dict, metadata: dict | None = None) -> float:
        """Record a tool invocation. Returns the timestamp for duration tracking."""
        ts = time.time()
        entry = ToolCallEntry(ts=ts, kind="call", tool_name=tool_name, args=args,
                               metadata=metadata or {})
        self._write(entry)
        return ts

    def log_result(self, tool_name: str, args: dict, result: Any,
                   start_ts: float, metadata: dict | None = None) -> None:
        duration_ms = (time.time() - start_ts) * 1000
        entry = ToolCallEntry(ts=time.time(), kind="result", tool_name=tool_name,
                               args=args, result=result, duration_ms=duration_ms,
                               metadata=metadata or {})
        self._write(entry)

    def log_error(self, tool_name: str, args: dict, error: Exception,
                  start_ts: float, metadata: dict | None = None) -> None:
        duration_ms = (time.time() - start_ts) * 1000
        entry = ToolCallEntry(ts=time.time(), kind="error", tool_name=tool_name,
                               args=args, error=repr(error), duration_ms=duration_ms,
                               metadata=metadata or {})
        self._write(entry)
    ...
    def wrap(self, tool_name: str, metadata: dict | None = None):
        ...
        def decorator(fn):
            @functools.wraps(fn)
            def wrapper(**kwargs):
                start = self.log_call(tool_name, kwargs, metadata)
                try:
                    result = fn(**kwargs)
                    self.log_result(tool_name, kwargs, result, start, metadata)
                    return result
                except Exception as exc:
                    self.log_error(tool_name, kwargs, exc, start, metadata)
                    raise
            return wrapper
        return decorator
